...
for prefix, next_hop in prefixes.items():
    curr_node = root
...
with open("var_decls.p4gen", "w") as f:
    for index, node in enumerate(nodes):
        if node != None:
            ...
            text = f"\nconst tbmnode_t nlevel_{tree_level}_{level_index} = {{0x{node.internalBitmap:04x}, 0x{node.externalBitmap:04x}, {node.outFaceArray}}};"
            f.write(text)
            # print(text)
